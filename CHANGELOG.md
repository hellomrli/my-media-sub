...
- **minisign 算法标记曾写反**（本版本内修正，未发布过）：`ED` 才是 minisign 默认的 BLAKE2b 预哈希模式，`Ed` 是旧式直接签名；写反会让所有官方 `minisign -S` 生成的签名验证失败。现在用 minisign 官方固件（预哈希与 legacy 各一条）锁住语义，并校验签名 key id 与公钥一致。`release.yml` 的签名条件改为读 job 级 `env` 中转的 secret（step 级 `if` 既不能引用 `secrets` 上下文——工作流文件会直接解析失败——也读不到 step 自身声明的 env，原写法会让签名永远被跳过）。更新下载的每一跳重定向都要求 `https`。
...
- 备份恢复不再直接替换运行中 Store 的文件。恢复请求校验并暂存归档，下一次启动在构造任何 Store 和后台任务之前应用，消除旧内存再次落盘覆盖恢复内容的问题。应用前刷新恢复快照以保留旧进程最后的写入；失败时回滚、停止启动并保留暂存归档，支持排除故障后重试。
- 编辑季度时将未标注季度的历史文件、集数键和探测记录绑定到原季度，再计算新主季进度，避免 S1 的完结记录误用于 S2。扩季与缩季继续保留历史转存证据。
- 检查、转存预览、任务载荷和持久化记录保留父目录季度信息，`Season 1/01.mkv` 与 `Season 3/01.mkv` 分别处理；新转存键使用 `s:<season>:ep:<episode>`，兼容读取旧主季 `ep:<episode>`。特典继续按文件名匹配，不占用正片集数。
- 详情网格按季度分组，统计所有选中季度；日历独立构建各季排期和处理状态，避免合并网格截断后丢失后续季度状态。下载任务 ID、目标目录和持久历史共同约束旧通知的季度归属，避免同名文件互相标记完成。
- 季度探测以链接和密码标识当前请求，允许新参数立即发起探测，并忽略过期结果；清空链接和重置编辑器会使在途请求失效。

### 升级与验证

- JSON Store schema 版本不变，可直接复用 v2.7.0 数据。备份恢复接口返回 `staged_files`、`restored_files: 0` 和 `restart_required: true`，实际恢复在重启时执行；详情 `episodes` 每项新增 `season`。
- 新增跨季编辑、同名文件转存与重载重试、独立日历和下载状态、备份恢复回滚重试、前端请求竞态回归测试。636 项 Rust 测试及 120 项前端测试通过，原有 2 项联网测试默认忽略。

## 2.7.0

### 新功能

- **跳季订阅**。订阅现在可以覆盖不连续的季度集合（例如只订 S1 与 S3，跳过 S2）。新增 `season_list` 字段保存集合语义，`season`/`season_end` 冗余存储为集合的最小/最大值以保持旧版本可读；恰好连续的集合折叠回区间语义，不产生冗余存储。检查、转存、重命名、批量修复命名与换源候选筛选全链路统一走 `subscription_file_matches_season`，与季度过滤同一口径，被跳过的中间季不会被转存。
- **分享季度探测**。新增 `POST /api/subscriptions/seasons`，探测分享链接中实际存在的季度，返回每季的视频文件数、识别到的集数与样本文件名。订阅编辑器把「手填季号」升级为「勾选检测到的季度」：粘贴链接或修改提取码后自动探测，单季自动选中，多季点击勾选（支持跳季）。整个分享都没有季度标记时（文件名只有 `01`、`02` 这类）按第一季处理并自动选中，UI 标注该季号为推断值提示用户核对；只要有任一文件带标记就不推断，避免混合分享（`Season 2/` 正片 + 根目录番外）凭空多出一个 S01。季度识别复用 `episode::season_hint_from_context`，与检查/转存的季度过滤完全同一实现，避免「检测出第 2 季、检查却过滤掉」的错位。
- 季号输入支持 `1`、`1-4`、`1,3`、`1,3-5` 混合语法（`parse_season_spec_list`）；`POST /api/utils/parse-season` 响应新增 `season_list`/`seasons` 字段；Telegram `/subscribe <序号> 1,3` 同步支持跳季，帮助文案已更新。

### 修复

- 多季/跳季订阅不再把最小季的单季集数回填为 `total_episode_number`，避免武装错误的完结目标并封顶其他季的转存（与 `backfill_total_episode_from_metadata` 统一口径）。
- 批量修复命名的 `{season}` 占位符改按文件自身识别的季号填充，跳季/多季订阅不再把所有文件都命名成最小季。
- 换源候选列表只保留可访问的资源：候选探测上限由 5 提升到 20（与搜索截断同值，并发全量探测），PanSou 已明确标记失效的候选直接丢弃，夸克探测确认失效（404/410、分享取消）的候选移除；尚未探测成功的（超时/暂时错误）保留并由前端标注，避免误伤慢响应的真实资源。
- 跳过转存的原因文案由「非当前订阅季」改为「不在订阅季度，暂不转存；编辑订阅勾选该季即可转存」，明确给出操作路径。

## 2.6.1

### 修复

- 订阅检查：多季订阅的去重集合改为按「季+集」限定，修复第二季同号集数被第一季 `known_episodes` 永久挡掉导致整季静默停更；起始集过滤只约束起始季；`known_episodes` 只写主季集数，避免跨季污染。
- 订阅检查：批量结果合并不再覆盖用户编辑的 `total_episode_number`，不再把旧 URL 的探测证据（`last_probe`/`check_history`/失效计数）写回并发编辑后的新链接，不再把并发转存刚写入的完结状态回退为失效。
- 集数识别：方括号年份 `[2024]`、编码/声道数字（H.265、DD5.1）、年份区间（2023-2024）不再误判为集数；SP/OVA/OAD 特典不占用正片集数槽位（仍按文件名转存）；订阅级 `episode_regex` 覆盖在自动检查/转存/去重全链路生效。
- 转存与下载：自动重命名补上 `portable_filename` 清洗（标题含 `: / "` 等字符时不再产出非法文件名）；多集包 `E01-E12` 记录全部集数键，分享方拆单集发布后不再重复转存；Aria2 失败任务不再被当作已提交复用，允许重新提交。
- 任务队列：恢复阶段改用非阻塞入队并加 Worker 空闲补扫，消除积压超过信号通道容量时进程启动挂死的可能；长转存期间发送心跳，看门狗不再把大批量转存误判为卡死（避免远端已成功后重试造成重复转存）。
- Telegram Bot：「转存 N / 订阅 N」确认时快照搜索结果指纹，确认期间会话被新搜索覆盖后按旧序号执行会被拦截；纯 long_polling 部署（未配置 webhook secret）现在也能生成「继续下载/查看详情」等主动按钮；切换到 disabled 时撤销 Telegram 侧 webhook；callback 消息缺失时明确应答；webhook 模式增加并发上限；长回复截断时明确提示。
- 安全：`trust_proxy_headers` 开启时登录限流键改取 `X-Forwarded-For` 最右侧（可信代理追加段），客户端伪造最左段不再能绕过限流；脱敏清单补齐 TMDB `api_key` 与夸克移动端 `kps`/`vcode`/`sign`，reqwest 错误入库前统一脱敏。
- 通知：摘要冲刷在安静时段/无可用路由时推迟而不是领取后丢弃，投递入队失败自动回滚领取；`subscription_invalid` 失效通知只发一次，来源恢复后再次失效才重新通知。
- 在线更新：升级包下载改用流式超时客户端（慢链路不再 30 秒必败）并增加体积硬上限；升级进度锁增加 panic 兜底；版本标签输入增加字符白名单。
- 客户端：夸克目录分页达到安全上限时显式标记结果可能不完整；上游 401/403 归类为「登录态失效，请更新 Cookie」而非临时故障；TMDB 详情增加 10 分钟缓存，重复搜索不再原样重放约 70 个上游请求。
- 前端：所有 API 请求默认 120 秒超时（长耗时操作可按需关闭），连接停滞不再永久卡死 loading 状态；网盘目录快速导航、订阅列表、任务列表、转存目录浏览增加请求序号防护，过期响应不再覆盖新状态；批量检查如实报告失败数量；搜索历史反序列化增加类型校验；清理订阅转存弹窗中不可达的手动转存死代码并修正确认按钮文案。

### 其他

- 换源回滚恢复换源前的起始集设置；检查历史 `transfer_count` 由转存服务真实回填；`/api/jobs?limit=` 增加钳制；docker-compose 增加 `stop_grace_period: 60s`；`TOTAL` 相关版本面随发布脚本同步。

## 2.6.0

### 新功能

- **Telegram 豆瓣链接转存 + 可选下载**。粘贴豆瓣链接搜索到夸克资源后，结果增加「转存 N」按钮（与「订阅 N」并存），点选后二次确认转存到夸克网盘根目录；转存完成后向 Telegram 推送「手动转存完成」通知并附带「继续下载」按钮，二次确认后拉取下载直链并提交 Aria2 下载到本地（下载目录默认取「剧集」目录，未配置则用 Aria2 默认目录）。同时支持 `/transfer <序号>` 命令。原有「订阅 N」能力保持不变。
- 转存任务结果新增落盘文件 fid 记录，供下载按钮回读；下载按钮回调复用现有签名与二次确认机制，任务 ID（UUID）作为回调资源，避免把大量 fid 塞进按钮数据。

### 清理

- **移除已下线的 STRM 残留**。STRM 能力自 v2.2.0 起已下线，本次彻底删除 `src/services/strm.rs` 服务、设置中的 `strm_*` 配置字段（`strm_enabled`/`strm_output_dir`/`strm_public_base_url`/`strm_access_token`/`strm_token_in_url`）、订阅与转存结果中的 STRM 字段，以及前端相关 UI。历史 `data/` 可直接复用，旧数据中的 STRM 字段会被 serde 自动忽略。
- 清理 v2.0.0 ~ v2.3.3 的旧版升级文档。

## 2.5.1

### 修复

- **夸克来源失效通知恢复**。夸克分享 token 或文件列表返回 HTTP 404/410 时现在会明确判定为失效，订阅会写入 `invalid` 状态并按通知设置发送 `subscription_invalid` 推送。
- 网络错误、429 限流和 5xx 上游故障仍按临时故障处理，避免将服务抖动误报为来源失效。
- 增加上游状态码保留、探测状态分类和失效通知回归测试。

## 2.5.0

### 修复

- **合并下载通知不再重复发送**。同一批次的多个文件在同一轮扫描中全部完成时，逐个任务使用最新去重快照，整批只发一条「下载完成」通知；此前同一轮里每个任务都会各发一条。升级重启后也不会重复补发：2.4.0 之前已按文件逐条通知过的批次，新版本会识别历史逐文件记录而跳过合并通知。
- 新增回归测试：同轮扫描去重、升级前逐文件历史去重，并验证测试确实能复现原缺陷。

### 媒体库元数据落盘（实验性，默认关闭）

- 下载完成后按匹配到的 TMDB 元数据写入媒体库元数据文件：剧集在 Aria2 下载目录生成 `tvshow.nfo`、`poster.jpg`、`backdrop.jpg`，`Season N` 目录生成 `season.nfo` 与季海报；电影生成 `movie.nfo`/`poster.jpg`/`backdrop.jpg`，供 Jellyfin/Emby/Kodi 刮削。
- NFO 为标准字段集（标题/原名/年份/简介/评分/首播日期/TMDB uniqueid），豆瓣来源不写 uniqueid；海报/背景图从 TMDB 下载并校验图片类型与大小上限。
- 文件写入幂等（字节相同跳过），重启回放自动重试；图片下载失败或目录不可写不阻塞下载流程，仅记录日志。
- 设置页「媒体元数据」新增开关（默认关闭），先试用再决定是否作为默认功能。

## 2.4.0

### 下载与推送

- Telegram 推送支持缩略图：推送时读取订阅海报，有图优先 `sendPhoto`，图片失败或 caption 超长时自动回退为文本消息。
- 同一次检查提交的多个 Aria2 下载文件，全部下载完成后合并为一条「下载完成」通知（列出文件名与大小），不再逐文件发送。
- Aria2 任务失败时立即发送「下载失败」通知（含文件名、目录与错误信息），活动中心新增 `download_failed` 事件标签；同批失败不会阻塞其余文件完成后的合并通知。
- 默认下载目录的年份括号由全角改为半角（`聪明镇（2026）` → `聪明镇 (2026)`），后端落盘、前端预览与表单示例同步更新。

### 设置与状态

- 打开项目时自动检测夸克账号状态（已配置 Cookie 时），侧边栏直接显示账号/签到/容量，无需手动点刷新。
- 系统设置概览新增「测试 TMDB API」按钮，通过新增的 `GET /api/metadata/test` 校验 TMDB Key（OpenAPI 同步至 93 路径 / 105 操作）。

- 前端构建校验、JS 测试、OpenAPI 与 Rust 检查全部通过。

## 2.3.3

### 日历卡片再调整

- 缩略图默认档位放大到 56×84，高屏 60×90 / 64×96，保持严格 2:3 海报比例。
- 卡片四边内边距从 6px 收窄到 3px，卡片高度从约 98px 降到约 92px（默认档）。
