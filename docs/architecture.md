...
The project is already async at the HTTP and service level. The next architectural step is to stop running long workflows directly inside request handlers and scheduler callbacks.
...
- `transfer.save`: save Quark share files, wait for eventual consistency, rename files.
...
2. Introduce a `jobs/` module with job models, `JobStore`, and an async worker loop.
3. Move manual transfer and subscription auto-transfer into jobs with persisted progress.
4. Add SSE endpoint for job progress and connect the WebUI transfer progress panel.
5. Split `static/index.html` only when the no-build WebUI becomes hard to maintain.
6. Add metadata scraping as a low-priority async job after core transfer UX is stable.
