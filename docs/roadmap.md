...
- **当前阶段**：v2.2.9 发布（订阅自动转存 / Aria2 回退 / 剧名清洗）
- **当前任务**：发布 v2.2.9（tag / Release 二进制 / GHCR）
- **下一任务（建议）**：换源可解释性 / Telegram 运维菜单 / 仪表盘失败归因；STRM 独立模块回归暂缓
- **后续阶段**：保持单实例安全模型与 JSON 单写；未经明确需求不启动多用户、SQLite 或第二 Provider
- **当前发布基线**：v2.2.9（`Cargo.toml` / OpenAPI / PWA 缓存代次）
...
- 2.2.8：Aria2 单批截断修复为分批提交全部文件。
- 2.2.9：自动转存门槛与 Aria2 提交回退；剧名去 emoji；重命名预览去重。
- 规划编号 P23+ 未正式立项；以产品目标驱动小版本，不再强制沿用旧 P 序号。
...
- Linux x86_64 归档为 7,590,367 bytes，SHA256 `36305c87cfda413c9a8dd2b2b76667bb09c146d036f64cfb8702460585f494b6`；下载后校验、内容清单和云构建二进制 smoke 均通过。
...
- v1.11.0 已正式发布：tag `v1.11.0`、GitHub Release、Linux x86_64 压缩包/SHA256 和 GHCR `1.11.0`/`1.11` 镜像均成功。
- main CI `29142009829` 通过审计、Release 构建、二进制/持续运行/真实浏览器/跨版本升级测试；tag Release `29142235784` 与 Docker `29142235785` 成功。
- P17 已完成；重启后从 P18-01 结构化日志关联开始，未经明确要求不创建新 tag 或 Release。
- P17 完整 main CI `29148432381` 已通过前端、格式、编译、Clippy、Rust 测试、RustSec、Release 二进制、持续运行、真实浏览器、Docker/二进制升级测试。
...
- `main` 已与 `origin/main` 同步，当前提交为 `3080208`。
- v1.9.0 已发布；后续 P11–P20 仅作为开发快照推进，未经明确要求不创建 tag/Release。
- P11 稳定性收口已完成：Release/升级/Docker/备份恢复/PWA/真实浏览器/短时持续运行测试均已接入。
- P12 追更准确性已完成：权威文件名语料、可解释识别、多集范围、SP/OVA/OAD、episode_regex 覆盖及缺集/重复集预览。
- 日历周/月/列表项目已支持剧照、季度海报和媒体海报缩略图回退。
- 明确冻结第二网盘和多用户计划；继续聚焦单用户夸克自动化。
- 下一阶段：P15 任务队列与调度可靠性；先实现任务优先级、公平调度和分层并发限制。
- 继续工作前先执行 `git status --short --branch`，不要回退或覆盖已推送成果。
...
- [x] `P11-03` 增加备份导出、恢复预览、恢复执行及篡改/错误确认阻断端到端测试。
- [x] `P11-04` 增加 PWA 跨版本缓存清理、离线壳层、认证安全，并用真实 Chrome 验证 390×844 日历/PWA 页面。
- [x] `P11-05` 增加持续健康/诊断请求、备份 Store 保留上限、损坏检测，并复用任务恢复与调度隔离测试。

## P12. 订阅追更准确性

- [x] `P12-01` 建立权威真实文件名语料，覆盖 SxxExx、EP、中文集数、方括号、独立数字、年份/清晰度误判和 SP/OVA/OAD。
- [x] `P12-02` 增加可解释集数识别结果，输出 method、confidence 和中文原因，并纳入检查详情。
- [x] `P12-03` 识别上下集、范围合集、多集文件及 SP/OVA/OAD，并在详情中返回完整 episodes 与 special_kind。
- [x] `P12-04` 提供订阅级 episode_regex 覆盖、命名捕获校验、安全回退及前端配置。
- [x] `P12-05` 转存预览返回逐文件完整集数、missing_episodes、duplicate_episodes 和最终操作/跳过原因。

## P13. 搜索与安全换源

- [x] 增加搜索关键词偏好、排除词、质量/字幕/编码偏好和稳定去重排序。
- [x] 增加候选 5 分钟探测缓存、PanSou 指数退避及换源候选集数/缺集覆盖预览。
- [x] 完善仅建议/自动应用模式、候选与成功通知、失败审计、历史回滚和冷却策略。
...
- [x] `P17-01` 优化移动端详情、批量操作、筛选持久化和大列表性能。
  - 订阅详情在 640px 下使用安全区、粘性导航/操作栏、44px 触控目标和单列弹窗；桌面与 390px 共享同一语义结构。
  - 订阅列表支持当前窗口全选、并发检查和带短语确认的批量删除；网盘已有批量选择继续复用统一确认。
  - 订阅状态/视图、Job 类型/状态、网盘类型/视图及通知筛选使用命名空间 localStorage 持久化并安全回退。
  - 订阅、Job、通知和网盘大列表使用分段可见窗口与“加载更多”，批处理使用受限并发池，避免一次渲染/请求全部项目。
- [x] `P17-02` 统一加载、空状态、错误边界、危险确认和键盘无障碍体验。
  - 全局 `aria-busy`/live loading bar、toast live region 和 window error/unhandledrejection 错误边界支持复制与关闭。
  - 删除订阅/网盘项目/下载记录、停止任务、清空通知、整理 Store、恢复备份和来源回滚统一使用可聚焦 alertdialog；高风险动作要求 DELETE/CLEAR/COMPACT/RESTORE DATA 短语。
  - Escape 关闭确认框、Enter 提交、Tab 焦点圈和现有 `/` 搜索快捷键统一由 Shell 生命周期管理。
- [x] `P17-03` 增加自动化时间线、诊断复制和真实浏览器端到端测试。
  - 结构化自动化事件按时间正序构建有界时间线，详情倒序展示并支持复制原始诊断 JSON；诊断页新增一键复制脱敏快照。
  - `scripts/smoke-browser.sh` 同时验证 Basic Auth API、Job/诊断接口、P17 DOM 合同及 390×844/1440×1000 Chrome 截图。
  - `static/js/core/ux.js` 的偏好、窗口、并发池、时间线和安全 JSON 有 Node 回归测试；PWA 缓存提升为 `v1.11.0-p17-1`。

P17 最终验证：389 个 Rust 测试登记，388 个通过、1 个真实 PanSou 网络测试按设计忽略；14 个前端 Node 测试及全部 JavaScript 语法检查通过；Tailwind CSS 重建、rustfmt、Clippy、完整 Rust 测试、真实浏览器 E2E 和 `git diff --check` 通过。
...
- [x] `P19-01` 增加备份校验清单、定期可恢复性验证和外部目录复制。
  - `BackupPreview.checks` 返回格式版本、Store schema、安全唯一路径、Base64/大小/SHA-256、业务模型和 settings 完整性的逐项 passed/warning 清单。
  - 每次服务器备份写入后先在 `backups/.verify-*` 隔离目录完整恢复，再逐文件复核大小与 SHA-256；临时目录无论成功失败均清理，绝不覆盖在线 DATA_DIR。
  - `BACKUP_VERIFY_INTERVAL_HOURS` 默认 24 小时，后台启动后立即验证最新备份并按周期重验；成功或失败报告原子写入 `backups/verification.json`。
  - `GET|POST /api/backups/verification` 提供最近报告和手动重验；诊断快照与 WebUI 展示最近状态、恢复文件数和校验清单。
...
- [x] `P21-01` 建立安全 Bot 接入、身份白名单与只读命令。
  ...
  - 证据：`src/services/telegram_bot.rs`、`src/api/telegram.rs`、Bot 设置/WebUI、`GET /api/diagnostics`、`docs/telegram-bot.md`、OpenAPI webhook 契约及单元/HTTP 集成测试。
  - 验证：413 个 Rust 测试登记，412 个通过、1 个真实 PanSou 网络测试按设计忽略；14 个前端 Node 测试通过；rustfmt、all-targets/all-features check、Clippy `-D warnings`、OpenAPI 路由契约、JavaScript 语法和 `git diff --check` 通过。
- [x] `P21-02` 增加受控写命令、交互确认与幂等执行。
  ...
  - 证据：`src/services/telegram_bot/{commands,tests}.rs`、`src/store/telegram_bot.rs`、`quark:signin` 最小 scope、确认绑定/过期/并发测试和通知已读复用测试。
- [x] `P21-03` 完成主动通知交互、审计、限流和端到端验证。
  ...
  - 证据：PushService HMAC 主动按钮、`GET /api/telegram/audits`、诊断页 Bot 卡片、三层限流/失败冷却、完整 DATA_DIR 备份校验、`scripts/smoke-telegram.sh` 与 CI 可选沙箱步骤。
