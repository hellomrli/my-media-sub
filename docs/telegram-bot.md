...
单实例、单管理员场景的 Telegram 控制入口：只读查询 + 受控写操作（确认按钮）+ 资源搜索/订阅/换源。与通知推送共用 `TELEGRAM_BOT_TOKEN`，控制面默认关闭；启用前必须配置数字 user/chat ID 白名单。
...
- 写操作（检查、订阅、换源、签到等）均需一次性确认按钮；不执行 Shell、不读取任意文件、不回显 Cookie/Token/密码。
- 搜索/换源会话写入 `telegram_bot.json`（15 分钟有效），可跨进程重启恢复；超时后请重新 `/search` 或 `/switch`。
...
4. 建议通过 BotFather 的 `/setcommands` 设置（Bot 启用后也会自动调用 `setMyCommands`）：
...
help - 命令列表
search - 搜索夸克资源
subscribe - 订阅搜索结果（需确认）
switch - 搜索换源候选
switch_apply - 应用换源（需确认）
status - 系统概况
subscriptions - 订阅列表
subscription - 订阅详情
calendar - 本周排期
...
直接向 Bot 发送豆瓣 subject 链接（例如 `https://movie.douban.com/subject/35445285/`）即可自动解析剧名并开始 PanSou 搜索，无需先发 `/search`；支持桌面版、移动版和带查询参数的链接。

直接发送中文也能唤起常用命令：`状态`、`订阅`、`任务`、`日历`、`通知`、`诊断`、`检查全部`、`帮助`；主菜单键盘也包含这些中文入口。

订阅、日历、任务和通知列表会附带“上一页 / 下一页”内联按钮，点击直接编辑当前消息切换页码，无需重新发送命令；列表缩到单页时按钮自动移除。

回复按 Unicode 字符边界切分，每条最多 3,500 字符，一次命令最多发送 4 条消息。未知命令只返回帮助，不会转发为系统命令。

消息统一使用 Telegram HTML 排版，来自 Store、搜索结果或上游错误的标题/链接/错误文本都会先转义再嵌入模板；推送渠道的标题和正文同样转义。极端情况下若 Telegram 仍拒绝解析，发送会去掉 `parse_mode` 自动重发，避免整条命令失败。
