...
impl DownloadMonitorService {
    ...
    pub fn start(self: Arc<Self>) {
        ...
        // 用 spawn_supervised 而不是裸 tokio::spawn：这个循环一旦 panic 就永久
        // 消失，下载完成通知、sync_downloads 完结、订阅自动完结以及 .nfo/海报
        // 副作用会全部静默停止，直到进程重启。
        let service = self.clone();
        crate::utils::spawn_supervised("Aria2 下载完成监控", move || {
            ...
            async move {
                ...
                loop {
                    ticker.tick().await;
                    if let Err(error) = service.poll_once(STOPPED_LIMIT).await {
