...
- 订阅数据刷新后会清理遗留的失败、重试和 `hidden` 状态，并为未成功加载的缩略图重新发起请求。
- PWA 缓存代次已提升，存储 schema 保持兼容，可从 v2.2.2 直接升级。

### 2.2.2

- 修复升级或重新登录后 Service Worker 继续返回旧版 JS/CSS、导致页面与脚本版本错配的问题。
- 远程海报和日历缩略图加载失败时自动进行两次退避重试，不再通过原生 `hidden` 状态永久隐藏。
- PWA 缓存版本与应用版本绑定，新增跨版本静态资源策略回归测试。
- 存储 schema 保持兼容，可从 v2.2.1 直接升级。
