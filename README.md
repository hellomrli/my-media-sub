...
My Media Sub 把资源搜索、订阅检查、更新日历、自动转存、重命名、STRM、Aria2 下载、失效换源和通知推送组合成一个可自托管的媒体自动化工作台。单二进制 + JSON 存储,无外部数据库。
...
| 下载与媒体库 | Aria2 幂等提交与退避重试、STRM 生成与 HTTPStrm 代理、Jellyfin/Emby/Plex/Webhook 刷新 |
...
定时器 / 手动检查
  ...
  → 夸克转存(成功即持久化) → 重命名 → STRM → Aria2 → 通知推送
