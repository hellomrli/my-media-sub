...
- 日历周视图订阅卡片：缩略图进一步放大（默认 56×84，保持 2:3），卡片四边内边距收窄到 3px，布局更紧凑。

### 2.3.2

- 日历周视图订阅卡片：缩略图改为严格 2:3 海报比例；文字列右移并放大一号，减少右侧留白。

### 2.3.1

- 日历周视图订阅卡片：缩略图略微放大（50×68，高屏分档增大），上下缝隙与左侧保持一致；卡片宽度收窄、文字右移，减少右侧留白。

### 2.3.0

- Telegram Bot 消息全面 HTML 化：加粗标题与 emoji 排版，所有用户/上游内容统一转义，非法 HTML 自动降级重发。
- 只读列表（订阅/任务/通知/日历）支持“上一页 / 下一页”内联按钮直接翻页。
- 支持中文指令与完整中文主菜单：状态、订阅、任务、日历、通知、诊断、检查全部、帮助。
- 直接粘贴豆瓣电影链接即可自动解析片名并搜索对应资源，无需先发 `/search`。
- 推送通道（Telegram / PushPlus）标题与正文先做 HTML 转义，避免特殊字符导致发送失败。

### 2.2.30

- 设置安全：保存前校验认证字段，拒绝空用户名、含冒号/控制字符的用户名与默认密码 `change-me`，避免把账号改到无法登录；用户名保存时自动去除首尾空白。
- 长期运行：网盘目录列表缓存写入前清理过期条目并设硬上限，消除无界内存增长。
- 发版工具：`bump-version.sh` 现在覆盖全部版本标签面（partials、service-worker.js、openapi.json）并重新生成 index.html，同时修复多行模板报错与重复标题问题。
- 清理：移除 `Config` 中从未被读取的认证字段死代码。
- 日历卡片：周视图缩略图留边并加细边框与圆角（不再撑满整卡）；「已存」行补上季度，如 `已存 S1 E10`。
