...
docker run -d \
  ...
  ghcr.io/hellomrli/my-media-sub:0.9.11
```
...
```bash
curl -LO https://github.com/hellomrli/my-media-sub/releases/download/v0.9.11/my-media-sub-v0.9.11-linux-x86_64.tar.gz
tar -xzf my-media-sub-v0.9.11-linux-x86_64.tar.gz
cd my-media-sub-v0.9.11-linux-x86_64
...
在线更新通过 GitHub Release 完成。服务会要求 Release 同时提供 Linux x86_64 包和对应 `.sha256` 文件，校验通过后才会解压和替换。
...
- 近期方向：保持 Rust 单体和夸克专精，优先完善追剧任务、规则中心、后台日志和工作台体验。

## 版本更新

### 0.9.11

- 工作台订阅总览的单条订阅项高度加倍，海报缩略图同步放大，提升首页订阅卡片的可读性。

### 0.9.10

- 工作台移除追更日历和待补集模块，减少首页信息噪音。
- 订阅总览卡片增高，订阅列表区域展示空间更充足。
- 清理对应的前端日历计算逻辑和样式，降低 WebUI 维护负担。

### 0.9.9

- WebUI 按 Sonarr/Radarr 类信息密度仪表盘风格重构，新增紧凑表格、状态徽章、细进度条和追更日历组件。
- 工作台升级为控制中心，展示 6 项 KPI、未来 7 天追更日历、待补集、订阅总览、夸克健康、后台任务和通知摘要。
- TMDB 元数据新增下一集和季内分集 `air_date`，批量刷新元数据后可为已有订阅补齐追更日历数据。
- 搜索、网盘、下载任务、订阅管理、后台日志和通知中心迁移到高密度表格；订阅管理新增表格/海报墙双视图。
- 通知中心继续将系统通知和推送结果合并展示，推送详情收进同一条通知的展开区。
- 设置页和弹窗整体收紧间距，移动端表格改为局部横向滚动，避免撑宽页面。
- 在线更新移除 `MY_MEDIA_SUB_ENABLE_SELF_UPDATE` 开关，默认启用 GitHub Release 在线更新能力。
