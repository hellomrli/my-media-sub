...
- 订阅管理：创建订阅后可手动或定时检查更新，订阅卡片展示刮削海报、名称、TMDB 评分、季度、集数进度和最后检查时间。
...
- 设置管理：支持运行时保存夸克 Cookie、推送配置、自定义订阅检查周期、NAS 同步、Aria2 等设置。
...
订阅模板可使用 `{}` 占位符。季号默认为 `0`，表示不写入季度；例如：
...
- `{title}`：订阅标题
- `{season}`：两位季号，例如 `02`；季号为 `0` 或未设置时为空
- `{season_number}`：原始季号，例如 `2`；季号为 `0` 或未设置时为空
- `{episode}`：两位集数，例如 `05`
