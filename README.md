...
```text
ghcr.io/hellomrli/my-media-sub:1.0.0
```
...
```bash
curl -LO https://github.com/hellomrli/my-media-sub/releases/download/v1.0.0/my-media-sub-v1.0.0-linux-x86_64.tar.gz
curl -LO https://github.com/hellomrli/my-media-sub/releases/download/v1.0.0/my-media-sub-v1.0.0-linux-x86_64.tar.gz.sha256
sha256sum -c my-media-sub-v1.0.0-linux-x86_64.tar.gz.sha256
tar -xzf my-media-sub-v1.0.0-linux-x86_64.tar.gz
cd my-media-sub-v1.0.0-linux-x86_64
...
| `QUARK_COOKIE` | 夸克网盘 Cookie，用于分享探测、转存、网盘管理和 HTTPStrm | 空 |
| `QUARK_SIGNIN_COOKIE` | 夸克签到 Cookie 或移动端签到请求 URL | 空 |
| `QUARK_SIGNIN_ENABLED` | 是否启用每日自动签到 | `false` |
| `QUARK_SIGNIN_HOUR` | 每日自动签到小时，范围 `0`-`23` | `8` |
| `PANSOU_API_URL` | PanSou API 地址 | 内置默认 |
...
| `WXPUSHER_APP_TOKEN` | WxPusher App Token | 空 |
| `WXPUSHER_UIDS` | WxPusher UID，逗号分隔 | 空 |
| `TELEGRAM_BOT_TOKEN` | Telegram Bot Token，仅用于推送 | 空 |
| `TELEGRAM_CHAT_ID` | Telegram Chat ID，仅用于推送 | 空 |
| `BARK_URL` | Bark 推送地址 | 空 |
| `GOTIFY_URL` | Gotify 地址 | 空 |
| `GOTIFY_TOKEN` | Gotify Token | 空 |
| `PUSHPLUS_TOKEN` | PushPlus Token | 空 |
| `SERVERCHAN_KEY` | Server 酱 SendKey | 空 |
...
`/strm/` 路由不走 Basic Auth，访问控制依赖 STRM Token。不要把 Token 暴露到不可信网络。
...
在线更新默认启用，不需要额外环境变量。Release 包必须同时提供 `.tar.gz` 和 `.sha256`，校验通过后才会替换二进制和静态资源。
...
- Docker 工作流：构建并推送 `v1.0.0`、`1.0.0`、`1.0` 和 SHA 标签镜像。
