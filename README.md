...
- 通过通知中心和推送渠道跟踪订阅更新、失效、完结、转存和下载状态。
...
| 重命名 | 模板命名、正则替换、重命名预览、已转存文件命名修复 |
| 元数据 | TMDB 自动匹配、手动候选选择、批量刮削、集数自动同步 |
| 网盘管理 | 夸克目录浏览、搜索、筛选、新建、重命名、删除、批量删除、每日签到 |
...
docker run -d \
  ...
  ghcr.io/hellomrli/my-media-sub:0.9.0
```
...
```bash
curl -LO https://github.com/hellomrli/my-media-sub/releases/download/v0.9.0/my-media-sub-v0.9.0-linux-x86_64.tar.gz
tar -xzf my-media-sub-v0.9.0-linux-x86_64.tar.gz
cd my-media-sub-v0.9.0-linux-x86_64
...
HTTPStrm 链接形如 `/strm/quark/{fid}/{file_name}?token=...`。媒体服务器访问该链接时，服务会用当前夸克 Cookie 实时换取临时下载地址并代理 Range 请求。`/strm/` 路由不走 Basic Auth，访问控制依赖 STRM Token，请不要把 Token 暴露到不可信环境。
...
文件名中识别到 `EP05`、`S01E05` 或 `第05集` 后，会生成类似：
...
在线更新通过 GitHub Release 完成：

- `GET /api/update/check`：检查最新版本和当前版本改动
- `GET /api/update/progress`：查询升级进度、阶段和百分比
- `POST /api/update/apply`：下载 Linux x86_64 Release 包，替换 `my-media-sub` 二进制和 WebUI 静态资源
...
前端没有单独构建步骤，WebUI 直接由 `static/index.html`、`static/app.js` 和 `static/styles.css` 提供。
